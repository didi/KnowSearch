package com.didi.arius.gateway.query.maker;
...
public class AggMaker {
    ...
    private AbstractAggregationBuilder makeCountAgg(MethodField field) {
        ...
        if ("DISTINCT".equals(field.getOption())) {
            CardinalityBuilder builders = AggregationBuilders.cardinality(field.getAlias()).field(field.getParams().get(0).value.toString());
            if (field.getParams().size()>1) {
                Integer precision_threshold = (Integer)(field.getParams().get(1).value);
                builders.precisionThreshold(precision_threshold);
            }
            return builders;
        }
