 ...
 */
@Component
public class DeleteIndexMoveStatsAction implements Action<String, Boolean> {
    protected static final Logger LOGGER = LoggerFactory.getLogger(DeleteIndexMoveStatsAction.class);
    @Value("${fastdump.httpTransport.port:8300}")
    private int                     httpPort;

    private final FutureUtil<Void> FUTURE_UTIL = FutureUtil.init("DeleteIndexMoveStatsAction-FutureUtil", 5, 5, 1000);
    @Autowired
    ...
    public Boolean doAction(String taskId) throws Exception {
            ...
            FUTURE_UTIL.runnableTask(() -> {
                ...
                try {
                    result = RetryUtil.retryWhenNullOrExceptionAndFailedThrowRuntimeException(
                            "DeleteIndexMoveStatsAction",
                            RetryUtil.DEFAULT_TIME,
                            RetryUtil.DEFAULT_INTERVAL_MILLS,
                            () -> {
                                String resp = BaseHttpUtil.deleteForString(buildHttpUrl(new String[] { ip, taskId }), null, buildHeader());
        ...
        if (err.length() > 0) {
            throw new FastDumpOperateException(String.format("触发成功, 部分节点上的索引shard执行失败, detail:%s", err));
        }
