...
public class RoutingIteratorTests extends ESAllocationTestCase {
    ...
    public void testReplicaShardPreferenceIters() throws Exception {
        ...
        for (String pref : removedPreferences) {
            GroupShardsIterator<ShardIterator> shardIterators = operationRouting.searchShards(clusterState, new String[]{"test"}, null, pref);
            assertWarnings("["+pref+"] has been removed; use [_only_nodes] or [_prefer_nodes]");
            assertEquals(shardIterators.size(), 2);
        }
    }
