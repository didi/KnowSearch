...
public class SearchTimeoutIT extends ESIntegTestCase {
    ...
    public void testHardSearchTimeout() throws Exception {
        String indexName = "search_timeout_index";
        prepareCreate(indexName).setSettings(Settings.builder()
            .put(IndexSettings.INDEX_GLOBAL_SEARCH_TIMEOUT_SWITCH, true)
            .build()).execute().actionGet();

        for (int i = 0; i < 32; i++) {
            client().prepareIndex(indexName, "type", Integer.toString(i)).setSource("field", "value").get();
        }
        refresh(indexName);

        ThrowingRunnable runnable = () -> {
            SearchResponse searchResponse = client().prepareSearch(indexName).setTimeout(new TimeValue(10, TimeUnit.MILLISECONDS))
                .setQuery(scriptQuery(
                    new Script(ScriptType.INLINE, "mockscript", SCRIPT_NAME, Collections.emptyMap())))
                .setAllowPartialSearchResults(true)
                .get();
    ...
    public void testPartialResultsIntolerantHardSearchTimeout() throws Exception {
        String indexName = "search_timeout_index";
        prepareCreate(indexName).setSettings(Settings.builder()
            .put(IndexSettings.INDEX_GLOBAL_SEARCH_TIMEOUT_SWITCH, true)
            .build()).execute().actionGet();

        client().prepareIndex(indexName, "type", "1").setSource("field", "value").setRefreshPolicy(IMMEDIATE).get();
