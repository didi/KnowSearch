...
public class BulkRequestParserTests extends ESTestCase {
    ...
    public void testDeleteRequest() throws IOException {
        BytesArray request = new BytesArray("{ \"delete\":{ \"_id\": \"bar\", \"pipeline\" : \"test\" } }\n");
        BulkRequestParser parser = new BulkRequestParser(randomBoolean());
        final AtomicBoolean parsed = new AtomicBoolean();
        parser.parse(request, "foo", null, null, null, null, false, XContentType.JSON,
                req -> fail(), req -> fail(),
                deleteRequest -> {
                    ...
                    assertEquals("test", deleteRequest.getPipeline());
                    parsed.set(true);
                });
        assertTrue(parsed.get());
    }

    public void testRemoveWithPipelineRequest() throws IOException {
        BytesArray request = new BytesArray("{ \"remove\":{ \"_id\": \"bar\", \"pipeline\" : \"test\" } }\n{}\n");
        BulkRequestParser parser = new BulkRequestParser(randomBoolean());
        final AtomicBoolean parsed = new AtomicBoolean();
        parser.parse(request, "foo", null, null, null, null, false, XContentType.JSON,
            req -> fail(), req -> fail(),
