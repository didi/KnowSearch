...
public class ClusterApplierService extends AbstractLifecycleComponent implements ClusterApplier {
    ...
    private static final Logger taskLogger = LogManager.getLogger("cluster.service.taskstate");
    ...
    class UpdateTask extends SourcePrioritizedRunnable implements Function<ClusterState, ClusterState> {
        ...
        UpdateTask(Priority priority, String source, ClusterApplyListener listener,
            ...
            this.taskState = new TaskState(state.get(), source, priority);
        }
    ...
    private void runTask(UpdateTask task) {
        ...
        task.taskState.setProcessTimeMillis(System.currentTimeMillis());
        ...
        try {
            ...
            task.taskState.setExecute(stopWatch.lastTaskTime().getMillis());
        } catch (Exception e) {
            ...
            task.taskState.setExecute(stopWatch.lastTaskTime().getMillis());
            logTaskState(task, "failure", e.getMessage(), previousClusterState == newClusterState);
            return;
        }
        ...
        if (previousClusterState == newClusterState) {
            ...
            logTaskState(task, "success", null, false);
        } else {
            ...
            try {
                applyChanges(task, previousClusterState, newClusterState, stopWatch, task.taskState);
                TimeValue executionTime = TimeValue.timeValueMillis(Math.max(0, currentTimeInMillis() - startTimeMS));
                logger.debug("processing [{}]: took [{}] done applying updated cluster state (version: {}, uuid: {})", task.source,
                    executionTime, newClusterState.version(),
                    newClusterState.stateUUID());
                warnAboutSlowTaskIfNeeded(executionTime, task.source, stopWatch);
                SafeClusterApplyListener listener = (SafeClusterApplyListener) task.listener;
                if (listener.listener instanceof RichClusterApplyListener) {
                    ((RichClusterApplyListener) task.listener).onSuccess(task.source, task.taskState);
                } else {
                    task.listener.onSuccess(task.source);
                }

                logTaskState(task, "success", null, true);
            } catch (Exception e) {
                TimeValue executionTime = TimeValue.timeValueMillis(Math.max(0, currentTimeInMillis() - startTimeMS));
                if (logger.isTraceEnabled()) {
                ...
                logTaskState(task, "failure", e.getMessage(), true);
            }
        }
    }

    private void logTaskState(UpdateTask task, String taskResult, String failure, boolean clusterStateChanged) {
        task.taskState.parseSourceToTaskType();
        task.taskState.setTaskResult(taskResult);
        if (Strings.hasText(failure)) {
            task.taskState.setFailure(failure);
        }
        task.taskState.setClusterStateChanged(clusterStateChanged);
        task.taskState.setProcess(System.currentTimeMillis() - task.taskState.getProcessTimeMillis());
        task.taskState.setTotal(System.currentTimeMillis() - task.taskState.getSubmitTimeMillis());
        String message = task.taskState.toString();
        if (Strings.hasText(message)) {
            taskLogger.info(message);
        }
    }

    private void applyChanges(UpdateTask task, ClusterState previousClusterState, ClusterState newClusterState, StopWatch stopWatch, TaskState taskState) {
        ...
        callClusterStateAppliers(clusterChangedEvent, stopWatch, taskState);
        ...
        callClusterStateListeners(clusterChangedEvent, stopWatch, taskState);
    }
    ...
    private void callClusterStateAppliers(ClusterChangedEvent clusterChangedEvent, StopWatch stopWatch, TaskState taskState) {
        clusterStateAppliers.forEach(applier -> {
            ...
            taskState.getAppliers().put(applier.getClass().getSimpleName(), stopWatch.lastTaskTime().millis());
        });
    }

    private void callClusterStateListeners(ClusterChangedEvent clusterChangedEvent, StopWatch stopWatch, TaskState taskState) {
        Stream.concat(clusterStateListeners.stream(), timeoutClusterStateListeners.stream()).forEach(listener -> {
            try {
                ...
                taskState.getListeners().put(listener.getClass().getSimpleName(), stopWatch.lastTaskTime().millis());
            } catch (Exception ex) {
    ...
    private static class SafeClusterApplyListener implements RichClusterApplyListener {
        ...
        @Override
        public void onFailure(String source, TaskState taskState, Exception e) {
            try {
                ((RichClusterApplyListener) listener).onFailure(source, taskState, e);
            } catch (Exception inner) {
                inner.addSuppressed(e);
                logger.error(new ParameterizedMessage(
                    "exception thrown by listener notifying of failure from [{}]", source), inner);
        ...
        @Override
        public void onSuccess(String source, TaskState taskState) {
            try {
                ((RichClusterApplyListener) listener).onSuccess(source, taskState);
            } catch (Exception e) {
                logger.error(new ParameterizedMessage(
                    "exception thrown by listener while notifying of cluster state processed from [{}]", source), e);
            }
