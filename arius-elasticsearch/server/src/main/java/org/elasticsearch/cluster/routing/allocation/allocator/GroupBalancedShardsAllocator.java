...
import static org.elasticsearch.cluster.metadata.IndexMetaData.INDEX_SETTING_DEFAULT_REGION_NAME;
import static org.elasticsearch.cluster.metadata.IndexMetaData.INDEX_SETTING_REGION_ATTRIBUTE;

/**
 * The {@link GroupBalancedShardsAllocator} re-balances the nodes allocations
 ...
 */
public class GroupBalancedShardsAllocator implements ShardsAllocator {

    private static final Logger logger = LogManager.getLogger(GroupBalancedShardsAllocator.class);

    public static final Setting<Float> INDEX_BALANCE_FACTOR_SETTING =
        Setting.floatSetting("cluster.routing.allocation.group.balance.index", 0.999f, 0.0f, Property.Dynamic, Property.NodeScope);
    public static final Setting<Float> SHARD_BALANCE_FACTOR_SETTING =
        Setting.floatSetting("cluster.routing.allocation.group.balance.shard", 0f, 0.0f, Property.Dynamic, Property.NodeScope);
    public static final Setting<Float> GROUP_BALANCE_FACTOR_SETTING =
        Setting.floatSetting("cluster.routing.allocation.group.balance.group", 0.001f, 0.0f, Property.Dynamic, Property.NodeScope);
    public static final Setting<Float> THRESHOLD_SETTING =
        Setting.floatSetting("cluster.routing.allocation.group.balance.threshold", 1.0f, 0.0f, Property.Dynamic, Property.NodeScope);

    private volatile WeightFunction weightFunction;
    private volatile float threshold;

    public GroupBalancedShardsAllocator(Settings settings) {
        // this(settings, new ClusterSettings(settings, ClusterSettings.BUILT_IN_CLUSTER_SETTINGS));
        setWeightFunction(INDEX_BALANCE_FACTOR_SETTING.get(settings), SHARD_BALANCE_FACTOR_SETTING.get(settings), GROUP_BALANCE_FACTOR_SETTING.get(settings));
        setThreshold(THRESHOLD_SETTING.get(settings));
    }

    public GroupBalancedShardsAllocator(Settings settings, ClusterSettings clusterSettings) {
        setWeightFunction(INDEX_BALANCE_FACTOR_SETTING.get(settings), SHARD_BALANCE_FACTOR_SETTING.get(settings), GROUP_BALANCE_FACTOR_SETTING.get(settings));
        setThreshold(THRESHOLD_SETTING.get(settings));
        clusterSettings.addSettingsUpdateConsumer(INDEX_BALANCE_FACTOR_SETTING, this::setWeightIndex);
        clusterSettings.addSettingsUpdateConsumer(SHARD_BALANCE_FACTOR_SETTING, this::setWeightShard);
        clusterSettings.addSettingsUpdateConsumer(GROUP_BALANCE_FACTOR_SETTING, this::setWeightGroup);
        clusterSettings.addSettingsUpdateConsumer(THRESHOLD_SETTING, this::setThreshold);
    }

    private void setWeightFunction(float indexBalance, float shardBalanceFactor, float groupBalanceFactor) {
        weightFunction = new WeightFunction(indexBalance, shardBalanceFactor, groupBalanceFactor);
    }

    private void setWeightIndex(float indexBalance) {
        weightFunction.indexBalance = indexBalance;
    }

    private void setWeightShard(float shardBalanceFactor) {
        weightFunction.shardBalance = shardBalanceFactor;
    }

    private void setWeightGroup(float groupBalanceFactor) {
        weightFunction.groupBalance = groupBalanceFactor;
    }
    ...
    public ShardAllocationDecision decideShardAllocation(final ShardRouting shard, final RoutingAllocation allocation) {
        ...
        if (shard.unassigned()) {
            ...
            moveDecision = balancer.decideMove(shard, Collections.newSetFromMap(new IdentityHashMap<>()));
    ...
    public static class WeightFunction {
        ...
        private float indexBalance;
        private float shardBalance;
        private float groupBalance;


        public WeightFunction(float indexBalance, float shardBalance, float groupBalance) {
            float sum = indexBalance + shardBalance + groupBalance;
            if (sum <= 0.0f) {
        ...
        private float weight(Balancer balancer, ModelNode node, String index, int numAdditionalShards) {
            ...
            Tuple<String, Float> group = balancer.getGroup(index);
            final float weightGroup = group == null ? 0 : (node.groupFactors(group.v1()) + numAdditionalShards * group.v2() - balancer.avgGroupPerNode(group.v1()));
            return indexBalance * weightIndex + shardBalance * weightShard + groupBalance * weightGroup;
        }
    }
    ...
    public static class Balancer {
        ...
        private final Map<String, Tuple<String, Float>> indicesGroup = new HashMap<>();
        private final Map<String, Float> groupTotalFactors = new HashMap<>();
        ...
        public float avgGroupPerNode(String group) {
            Float totalFactor = groupTotalFactors.get(group);
            if (totalFactor != null) {
                return groupTotalFactors.get(group) / nodes.size();
            } else {
                return 0f;
            }
        }

        public Tuple<String, Float> getGroup(String index) {
            return indicesGroup.get(index);
        }
        ...
        private NodeSorter newNodeSorter(List<RoutingNode> routingNodeList) {
            List<ModelNode> modelNodeList = routingNodeList.stream().map(RoutingNode::getModelNode).collect(Collectors.toList());
            return new NodeSorter(modelNodeList.toArray(new ModelNode[modelNodeList.size()]), weight, this);
        }

        private void newGroupInfo() {
            metaData.indices().forEach(entry -> {
                String group = entry.value.getSettings().get(IndexGroupSettings.INDEX_GROUP_NAME);
                float shardFactor = entry.value.getSettings().getAsFloat(IndexGroupSettings.INDEX_GROUP_FACTOR, 0.1f);

                if (group != null) {
                    indicesGroup.put(entry.key, new Tuple<>(group, shardFactor));
                    Float totalFactor = groupTotalFactors.get(group);
                    if (totalFactor != null) {
                        groupTotalFactors.put(group, totalFactor + shardFactor * entry.value.getTotalNumberOfShards());
                    } else {
                        groupTotalFactors.put(group, shardFactor * entry.value.getTotalNumberOfShards());
                    }
                }
            });
        }
        ...
        private void balanceByWeights() {
            Tuple<String, List<RoutingNode>> tuple = allocation.routingNodes().randomRegion();
            if (tuple.v2().size() < 2) {
                if (logger.isTraceEnabled()) {
                    logger.trace("skipping rebalance as region single node only");
                }
                return;
            }
            sorter = newNodeSorter(tuple.v2());
            final Set<ModelNode> throttledNodes = Collections.newSetFromMap(new IdentityHashMap<>());
            final AllocationDeciders deciders = allocation.deciders();
            final ModelNode[] modelNodes = sorter.modelNodes;
            final float[] weights = sorter.weights;
            for (String index : buildWeightOrderedIndices(tuple.v1())) {
                ...
                while (true) {
                    ...
                    if (maxNode.numShards(index) > 0) {
                        ...
                        Type type = tryRelocateShard(minNode, maxNode, index, delta, throttledNodes);
                        if (type == Type.THROTTLE || throttledNodes.size() >= tuple.v2().size()) {
                            return;
                        }
                        if (type == Type.YES) {
                            /*
         ...
         */
        private String[] buildWeightOrderedIndices(String regionName) {
            List<String> indicesByRegion = Arrays.stream(allocation.routingTable().indicesRouting().keys().toArray(String.class)).filter(index -> {
                if (metaData.hasIndex(index)) {
                    IndexMetaData indexMetaData = metaData.index(index);
                    Settings settings = indexMetaData.getSettings();
                    final Map<String, String> requireMap = INDEX_ROUTING_REQUIRE_GROUP_SETTING.getAsMap(settings);
                    final Map<String, String> includeMap = INDEX_ROUTING_INCLUDE_GROUP_SETTING.getAsMap(settings);
                    String require = requireMap.getOrDefault(INDEX_SETTING_REGION_ATTRIBUTE, INDEX_SETTING_DEFAULT_REGION_NAME);
                    String include = includeMap.getOrDefault(INDEX_SETTING_REGION_ATTRIBUTE, INDEX_SETTING_DEFAULT_REGION_NAME);
                    if (regionName.equals(INDEX_SETTING_DEFAULT_REGION_NAME)) {
                        if (require.equals(INDEX_SETTING_DEFAULT_REGION_NAME) && include.equals(INDEX_SETTING_DEFAULT_REGION_NAME)) {
                            return true;
                        }
                    } else {
                        if (require.equals(regionName) || include.equals(regionName)) {
                            return true;
                        }
                    }
                }
            ...
            final String[] indices = indicesByRegion.toArray(new String[indicesByRegion.size()]);
        ...
        public void moveShards() {
            ...
            Tuple<String, List<RoutingNode>> tuple = allocation.routingNodes().randomRegion();
            final Set<ModelNode> throttledNodes = Collections.newSetFromMap(new IdentityHashMap<>());
            for (Iterator<ShardRouting> it = allocation.routingNodes().nodeInterleavedShardIterator(tuple.v2()); it.hasNext(); ) {
                ShardRouting shardRouting = it.next();
                final MoveDecision moveDecision = decideMove(shardRouting, throttledNodes);
                if (moveDecision.isDecisionTaken() && moveDecision.forceMove()) {
                    final ModelNode sourceNode = nodes.get(shardRouting.currentNodeId());
                    final ModelNode targetNode = nodes.get(moveDecision.getTargetNode().getId());
                    sourceNode.removeShard(shardRouting, metaData);
                    ...
                    targetNode.addShard(relocatingShards.v2(), metaData);
                    if (logger.isTraceEnabled()) {
                ...
                if (throttledNodes.size() >= tuple.v2().size()) {
                    break;
                }
            }
        }

        public MoveDecision decideMove(final ShardRouting shardRouting, final Set<ModelNode> throttledNodes) {
            ...
            for (ModelNode currentNode : sorter.modelNodes) {
                if (currentNode != sourceNode) {
                    if ((throttledNodes.contains(currentNode) || currentNode.containsShard(shardRouting)) && explain == false) {
                    ...
                    if (allocationDecision.type() == Type.THROTTLE) {
                        throttledNodes.add(currentNode);
                    }
        ...
        private Map<String, ModelNode> buildModelFromAssigned() {
            ...
            for (RoutingNode rn : routingNodes) {
                nodes.put(rn.nodeId(), rn.getModelNode());
            }
            return nodes;
        }
            ...
            do {
                for (int i = 0; i < primaryLength; i++) {
                    ...
                    if (allocationDecision.getAllocationDecision() == AllocationDecision.YES) {
                        ...
                        minNode.addShard(shard, metaData);
                        if (!shard.primary()) {
                        ...
                        if (minNode != null) {
                            ...
                            final long shardSize = DiskThresholdDecider.getExpectedShardSize(shard,
                                ShardRouting.UNAVAILABLE_EXPECTED_SHARD_SIZE,
                                allocation.clusterInfo(), allocation.metaData(), allocation.routingTable());
                            minNode.addShard(shard.initialize(minNode.getNodeId(), null, shardSize), metaData);
        ...
        private AllocateUnassignedDecision decideAllocateUnassigned(final ShardRouting shard, final Set<ModelNode> throttledNodes) {
            ...
            for (ModelNode node : nodes.values()) {
                ...
                if (currentDecision.type() == Type.YES || currentDecision.type() == Type.THROTTLE) {
                    if (currentDecision.type() == Type.THROTTLE) {
                        throttledNodes.add(node);
                        continue;
                    }
         ...
         */
        private Type tryRelocateShard(ModelNode minNode, ModelNode maxNode, String idx, float minCost, final Set<ModelNode> throttledNodes) {
            if (throttledNodes.contains(minNode) || throttledNodes.contains(maxNode)) {
                // decision is NO without needing to check anything further, so short circuit
                return Type.NO;
            }
            ...
            if (index != null) {
                ...
                for (ShardRouting shard : index) {
                    if (shard.started()) {
                        ...
                        if (rebalanceDecision.type() == Type.THROTTLE) {
                ...
                if (candidate != null) {
                    ...
                    maxNode.removeShard(candidate, metaData);
                    ...
                    if (decision.type() == Type.YES) { /* only allocate on the cluster if we are not throttled */
                        ...
                        minNode.addShard(routingNodes.relocateShard(candidate, minNode.getNodeId(), shardSize, allocation.changes()).v1(), metaData);
                        return Type.YES;
                    } else {
                        assert decision.type() == Type.THROTTLE;
                        minNode.addShard(candidate.relocate(minNode.getNodeId(), shardSize), metaData);
                    }
                }
            }
            if (logger.isTraceEnabled()) {
    ...
    public static class ModelNode implements Iterable<ModelIndex> {
        ...
        private final Map<String, Float> groupFactors = new HashMap<>();
        ...
        public float groupFactors(String group) {
            Float factor = groupFactors.get(group);
            return factor == null ? 0 : factor;
        }
        ...
        public void addShard(ShardRouting shard, MetaData metaData) {
            ...
            // add group factor
            addGroupFactor(shard, metaData);
        }

        public void removeShard(ShardRouting shard, MetaData metaData) {
            ModelIndex index = indices.get(shard.getIndexName());
            if (index != null) {
            ...
            // remove group factor
            removeGroupFactor(shard, metaData);
        }

        private void addGroupFactor(ShardRouting shard, MetaData metaData) {
            // add group factor
        ...
        private void removeGroupFactor(ShardRouting shard, MetaData metaData) {
            IndexMetaData indexMetaData = metaData.index(shard.getIndexName());
            float shardFactor = indexMetaData.getSettings().getAsFloat(IndexGroupSettings.INDEX_GROUP_FACTOR, 0f);
            String group = indexMetaData.getSettings().get(IndexGroupSettings.INDEX_GROUP_NAME);
            ...
            if (group != null) {
                Float totalFactor = groupFactors.get(group);
                if (totalFactor != null && totalFactor >= shardFactor) {
                    groupFactors.put(group, totalFactor - shardFactor);
                } else {
                    groupFactors.put(group, 0f);
                }
            }
        }
