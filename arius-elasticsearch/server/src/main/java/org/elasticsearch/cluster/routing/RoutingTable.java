...
public class RoutingTable implements Iterable<IndexRoutingTable>, Diffable<RoutingTable> {
    ...
    public Builder cloneBuilder() {
        return new Builder().deepCopy(this);
    }
    ...
    public static class Builder {
        ...
        public Builder deepCopy(RoutingTable routingTable) {
            version = routingTable.version;
            for (IndexRoutingTable indexRoutingTable : routingTable) {
                indicesRouting.put(indexRoutingTable.getIndex().getName(), indexRoutingTable.cloneBuilder().build());
            }
            return this;
        }

        public void updateShard(ShardRouting source, ShardRouting target) {
            IndexRoutingTable.Builder builder = IndexRoutingTable.builder(source.index());
            IndexRoutingTable indexRoutingTable = indicesRouting.remove(source.getIndexName());
            if (indexRoutingTable != null) {
                for (IndexShardRoutingTable shardRoutingTable : indexRoutingTable) {
                    if (shardRoutingTable.shardId.id() == source.id()) {
                        for (ShardRouting shardRouting : shardRoutingTable) {
                            if (shardRouting == source) {
                                continue;
                            }
                            // every relocating shard has a double entry
                            if (source.initializing()
                                && source.relocatingNodeId() != null
                                && shardRouting.relocating()
                                && source.relocatingNodeId().equals(shardRouting.currentNodeId())) {
                                continue;
                            }
            ...
            if (target != null) {
                builder.addShard(target);
            }
            indexRoutingTable = builder.build();
            if (indexRoutingTable.shards().isEmpty() == false) {
                indicesRouting.put(source.getIndexName(), indexRoutingTable);
            }
        }
