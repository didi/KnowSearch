...
public class IndexRoutingTable extends AbstractDiffable<IndexRoutingTable> implements Iterable<IndexShardRoutingTable> {
    ...
    IndexRoutingTable(Index index, ImmutableOpenIntMap<IndexShardRoutingTable> shards) {
        ...
        for (IntObjectCursor<IndexShardRoutingTable> cursor : shards) {
            for (ShardRouting shardRouting : cursor.value) {
                ...
                this.stateShards.computeIfAbsent(shardRouting.state(), k -> new ArrayList<>()).add(shardRouting);
                if (shardRouting.relocating()) {
                    ShardRouting routingEntry = shardRouting.getTargetRelocatingShard();
                    this.stateShards.computeIfAbsent(routingEntry.state(), k -> new ArrayList<>()).add(routingEntry);
                }
            }
        }
    ...
    public List<ShardRouting> shardsWithState(ShardRoutingState state) {
        return stateShards.getOrDefault(state, new ArrayList<>());
    }
    ...
    public Builder cloneBuilder() {
        return new Builder(index).deepCopy(this);
    }
    ...
    public static class Builder {
        ...
        public Builder deepCopy(IndexRoutingTable indexRoutingTable) {
            for (IndexShardRoutingTable indexShardRoutingTable : indexRoutingTable) {
                shards.put(indexShardRoutingTable.shardId.id(), new IndexShardRoutingTable.Builder(indexShardRoutingTable).build());
            }
            return this;
        }
