...
public class OperationRouting {
    ...
    public static final String ROUTING_VALUE_SPLIT = "!";
        ...
        for (String index : concreteIndices) {
            ...
            if (effectiveRouting != null) {
                for (String r : effectiveRouting) {
                    if (indexMetaData.isRoutingSize()) {
                        int routingSlots = indexMetaData.getNumberOfShards() / indexMetaData.getNumberOfRoutingSize();
                        for (int slot = 0; slot < routingSlots; slot++) {
                            set.add(RoutingTable.shardRoutingTable(indexRouting, calculateRoutingSizeShardId(indexMetaData, r, slot)));
                        }
                    } else {
    ...
    private ShardIterator preferenceActiveShardIterator(IndexShardRoutingTable indexShard, String localNodeId,
        ...
        if (preference.charAt(0) == '_') {
            ...
            switch (preferenceType) {
                ...
                case PRIMARY:
                    deprecationLogger.deprecated("[_primary] has been removed; " +
                        "use [_only_nodes] or [_prefer_nodes]");
                    return shardRoutings(indexShard, nodes, collectorService, nodeCounts);
                case REPLICA:
                    deprecationLogger.deprecated("[_replica] has been removed; " +
                        "use [_only_nodes] or [_prefer_nodes]");
                    return shardRoutings(indexShard, nodes, collectorService, nodeCounts);
                case PRIMARY_FIRST:
                    deprecationLogger.deprecated("[_primary_first] has been removed; " +
                        "use [_only_nodes] or [_prefer_nodes]");
                    return shardRoutings(indexShard, nodes, collectorService, nodeCounts);
                case REPLICA_FIRST:
                    deprecationLogger.deprecated("[_replica_first] has been removed; " +
                        "use [_only_nodes] or [_prefer_nodes]");
                    return shardRoutings(indexShard, nodes, collectorService, nodeCounts);
                default:
    ...
    public static int generateShardId(IndexMetaData indexMetaData, @Nullable String id, @Nullable String routing) {
        ...
        if (indexMetaData.isRoutingSize()
            && id != null) {
            int pos = id.lastIndexOf(ROUTING_VALUE_SPLIT);
            if (pos > 0 && pos + 1 < id.length()) {
                String randomShardsRouting = id.substring(pos + 1);
                int routingSlot = Integer.valueOf(randomShardsRouting);
                return calculateRoutingSizeShardId(indexMetaData, effectiveRouting, routingSlot);
            }
        }
    ...
    public String generateRoutingSizeId(IndexMetaData indexMetaData, String id, String routingValue) {
        int routingSlot = calculateRoutingSlot(indexMetaData, routingValue);
        return id + ROUTING_VALUE_SPLIT + routingSlot;
    }
    ...
    public static int calculateRoutingSlot(IndexMetaData indexMetaData, String routingValue) {
        return Math.floorMod(Murmur3HashFunction.hash(routingValue), (indexMetaData.getNumberOfShards() / indexMetaData.getNumberOfRoutingSize()));
    }

    private static int calculateRoutingSizeShardId(IndexMetaData indexMetaData, String effectiveRouting, int routingSlot) {
        return Math.floorMod(Murmur3HashFunction.hash(effectiveRouting), indexMetaData.getNumberOfRoutingSize())
            + indexMetaData.getNumberOfRoutingSize() * routingSlot;
    }
