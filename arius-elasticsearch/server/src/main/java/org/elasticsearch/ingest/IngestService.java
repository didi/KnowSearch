...
public class IngestService implements ClusterStateApplier {
    ...
    public Map<String, PipelineHolder> pipelines() {
        return pipelines;
    }
        ...
        threadPool.executor(ThreadPool.Names.WRITE).execute(new AbstractRunnable() {
            ...
            protected void doRun() {
                ...
                for (DocWriteRequest<?> actionRequest : actionRequests) {
                    ...
                    executePipelines(i, pipelines.iterator(), indexRequest, onDropped, onFailure);
                    postExecute(actionRequest, indexRequest);
        ...
        while (it.hasNext()) {
            ...
            try {
                ...
                innerExecute(slot, indexRequest, pipeline, onDropped, e -> {
                    ...
                    if (it.hasNext()) {
                        executePipelines(slot, it, indexRequest, onDropped, onFailure);
                    }
                });
            } catch (Exception e) {
                onFailure.accept(slot, e);
    ...
    public static class PipelineHolder {
        ...
        public final PipelineConfiguration configuration;
    ...
    private void postExecute(DocWriteRequest docWriteRequest, IndexRequest indexRequest) {
        if (docWriteRequest instanceof UpdateRequest) {
            UpdateRequest updateRequest = (UpdateRequest) docWriteRequest;
            IndexRequest docIndexRequest = updateRequest.docAsUpsert() ? updateRequest.doc() : updateRequest.upsertRequest();
            if (docIndexRequest == null) {
                docIndexRequest = updateRequest.doc();
            }

            if (docIndexRequest != null) {
                updateRequest.index(docIndexRequest.index());
                updateRequest.id(docIndexRequest.id());
                updateRequest.routing(docIndexRequest.routing());
            }
        } else if (docWriteRequest instanceof DeleteRequest) {
            DeleteRequest deleteRequest = (DeleteRequest) docWriteRequest;
            if (deleteRequest.source() != null) {
                deleteRequest.index(indexRequest.index());
                deleteRequest.id(indexRequest.id());
                deleteRequest.routing(indexRequest.routing());
                deleteRequest.version(indexRequest.version());
                deleteRequest.versionType(indexRequest.versionType());
                deleteRequest.setPipeline(indexRequest.getPipeline());
            }
        }
    }
}
