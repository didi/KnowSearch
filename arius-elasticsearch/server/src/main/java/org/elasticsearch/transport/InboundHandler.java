    ...
    private void messageReceived(BytesReference reference, TcpChannel channel) throws IOException {
        ...
        try (ThreadContext.StoredContext existing = threadContext.stashContext();
            ...
            if (message.isRequest()) {
                action = handleRequest(channel, (InboundMessage.Request) message, reference.length());
            } else {
                ...
                if (message.isHandshake()) {
                    ...
                    TcpTransport.ResponseContext<? extends TransportResponse> context = responseHandlers.remove(requestId);
                    messageListener.onResponseReceived(requestId, context);
                    if (context == null && message.isError()) {
                        handler = handshaker.removeHandlerForHandshake(requestId);
                        action = "hand_shake";
            ...
            String outRequestId = threadPool.getThreadContext().getHeader("requestId");
            RequestTracker.getInstance().logTcpRequest(outRequestId, requestId, action, message.isRequest(), remoteAddress.toString(), start);
        }
    }

    private <T extends TransportRequest> String handleRequest(TcpChannel channel, InboundMessage.Request message, int messageLengthBytes) {
