...
public abstract class TransportReplicationAction<
    ...
    final class ReroutePhase extends AbstractRunnable {
        ...
        ReroutePhase(ReplicationTask task, Request request, ActionListener<Response> listener) {
            ...
            // 屏蔽该设置，防止task覆盖bulkItem请求
            if (task != null && !request.getParentTask().getNodeId().equals(clusterService.localNode().getId())) {
                this.request.setParentTask(clusterService.localNode().getId(), request.getParentTask().getId());
            }
