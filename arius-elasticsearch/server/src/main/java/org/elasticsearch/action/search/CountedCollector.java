...
final class CountedCollector<R extends SearchPhaseResult> {
    ...
    private final AtomicBoolean hasRunFinishPhase = new AtomicBoolean(false);
    ...
    void countDown() {
        ...
        if (context.getTask() != null && context.getTask().isCancelled()) {
            if (hasRunFinishPhase.compareAndSet(false, true)) {
                onFinish.run();
            }
            return;
        }
