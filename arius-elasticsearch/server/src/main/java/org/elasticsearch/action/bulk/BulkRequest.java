...
public class BulkRequest extends ActionRequest implements CompositeIndicesRequest, WriteRequest<BulkRequest> {
    ...
    public BulkRequest(Map<String, List<String>> headers) {
        super(headers);
    }
    ...
    public String getLogPipelineId() {
        return logPipelineId;
    }

    public void setLogPipelineId() {
        if(logPipelineId!=null) {
            return;
        }

        String ppid = pipeline();
        if(ppid==null) {
            for(DocWriteRequest dwr : requests()) {
                ...
                if(dwr instanceof DeleteRequest) {
                    ppid = ((DeleteRequest) dwr).getPipeline();
                }

                if(dwr instanceof UpdateRequest) {
                    IndexRequest ir = ((UpdateRequest) dwr).upsertRequest();
                    if(ir!=null) {
                        ppid = ir.getPipeline();
                    }
                }
                ...
                if(dwr instanceof IndexRequest) {
                    ppid = ((IndexRequest) dwr).getPipeline();
                }
        ...
        if("_none".equalsIgnoreCase(ppid)) {
            ppid = null;
        }

        this.logPipelineId = ppid;
    }
}
