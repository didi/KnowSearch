...
public final class IndexingSlowLog implements IndexingOperationListener {
    ...
    public void postIndex(ShardId shardId, Engine.Index indexOperation, Engine.IndexResult result) {
        if (result.getResultType() == Engine.Result.Type.SUCCESS) {
            ...
            if (indexWarnThreshold >= 0 && tookInNanos > indexWarnThreshold) {
                indexLogger.warn( new IndexingSlowLogMessage(shardId, doc, tookInNanos, reformat, maxSourceCharsToLog));
            } else if (indexInfoThreshold >= 0 && tookInNanos > indexInfoThreshold) {
                indexLogger.info(new IndexingSlowLogMessage(shardId, doc, tookInNanos, reformat, maxSourceCharsToLog));
            } else if (indexDebugThreshold >= 0 && tookInNanos > indexDebugThreshold) {
                indexLogger.debug(new IndexingSlowLogMessage(shardId, doc, tookInNanos, reformat, maxSourceCharsToLog));
            } else if (indexTraceThreshold >= 0 && tookInNanos > indexTraceThreshold) {
                indexLogger.trace( new IndexingSlowLogMessage(shardId, doc, tookInNanos, reformat, maxSourceCharsToLog));
            }
        }
    }
    ...
    static final class IndexingSlowLogMessage extends ESLogMessage {
        ...
        IndexingSlowLogMessage(ShardId shardId, ParsedDocument doc, long tookInNanos, boolean reformat, int maxSourceCharsToLog) {
            super(prepareMap(shardId,doc,tookInNanos,reformat,maxSourceCharsToLog),
                message(shardId,doc,tookInNanos,reformat,maxSourceCharsToLog));
        }

        private static Map<String, Object> prepareMap(ShardId shardId, ParsedDocument doc, long tookInNanos, boolean reformat,
                                                      int maxSourceCharsToLog) {
            Map<String,Object> map = new HashMap<>();
            map.put("message", shardId);
        ...
        private static String message(ShardId shardId, ParsedDocument doc, long tookInNanos, boolean reformat, int maxSourceCharsToLog) {
