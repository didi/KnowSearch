...
public class PackageDomainServiceImpl implements PackageDomainService {
    ...
    public Result<Void> createPackage(Package pk) {
        //确保数据库中某个软件包类型的安装包名称+版本唯一
        if (null != packageRepository.findByUniqueKey(pk.getName(), pk.getVersion(), pk.getPackageType())) {
            return Result.fail(ResultCode.PARAM_ERROR.getCode(),"安装包名称和版本已存在，请重新输入");
        }
        //创建
        pk.create();
    ...
    @Override
    public List<PackageGroupConfig> listPackageGroupConfigByKey(String name, String version, Integer packageType) {
        Package packageByName = packageRepository.findByUniqueKey(name, version, packageType);
