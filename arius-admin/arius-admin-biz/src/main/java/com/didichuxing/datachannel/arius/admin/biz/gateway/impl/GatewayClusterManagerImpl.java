...
public class GatewayClusterManagerImpl implements GatewayClusterManager {
	...
	public Result<GatewayClusterVO> join(GatewayClusterJoinDTO param, Integer projectId,
		...
		public Result<Void> createWithECM(GatewayClusterCreateDTO gatewayCluster, Integer projectId,
				...
				//要处理一种情况，假如上报速度大于任务的回写速度，那么注定会导致这里插件失败，所以如果找到了，则默认走更新接口
				final Optional<GatewayClusterPO> clusterPOOptional = gatewayClusterService.listByNames(
						Collections.singletonList(gatewayDTO.getClusterName())).stream().findFirst();
				if (clusterPOOptional.isPresent()) {
						final Integer id = clusterPOOptional.get().getId();
						gatewayDTO.setId(id);
						final boolean update = gatewayClusterService.update(gatewayDTO);
						if (update) {
								final List<String> keys = gatewayNodeService.listByClusterName(
												gatewayDTO.getClusterName())
										.stream()
										.map(i -> String.format("%s:%s",
												i.getHostName(), i.getPort()))
										.collect(Collectors.toList());
								final List<GatewayNodeHostDTO> nodes = gatewayCluster.getNodes().stream()
										.filter(i -> {
										...
										.collect(Collectors.toList());
								//3. 写入节点信息
								final boolean addGatewayNode = gatewayNodeService.insertBatch(
										nodes);
								if (!addGatewayNode) {
										TransactionAspectSupport.currentTransactionStatus()
												.setRollbackOnly();
										return Result.buildFail("创建失败");
								}
						}
						return Result.build(update);
				}
				
				final boolean addGatewayCluster = gatewayClusterService.insertOne(gatewayDTO);
				if (addGatewayCluster) {
						//3. 写入节点信息
						final boolean addGatewayNode = gatewayNodeService.insertBatch(
								gatewayCluster.getNodes());
						if (!addGatewayNode) {
								TransactionAspectSupport.currentTransactionStatus()
										.setRollbackOnly();
								return Result.buildFail("创建失败");
				...
				return Result.build(addGatewayCluster);
		}
