...
public class ClusterContextManagerImpl implements ClusterContextManager {
    ...
    private void flushClusterPhyContexts() {
        ...
        // host信息按【cluster】分组
        List<RoleClusterHostInfo> roleClusterHostInfos = roleClusterHostInfoService.listAllNode();
        Map<String, List<RoleClusterHostInfo>> cluster2RoleListMap = ConvertUtil.list2MapOfList(roleClusterHostInfos,
                RoleClusterHostInfo::getCluster, RoleClusterHostInfo -> RoleClusterHostInfo);
        ...
        for (ClusterPhy phy : clusterPhyList) {
            ...
            List<RoleClusterHostInfo> hostList = cluster2RoleListMap.get(phy.getCluster());
            ...
            List<RoleClusterHostInfo> dataNodes = hostList.stream().filter(r -> DATA_NODE.getCode() == r.getRole())
                    .collect(Collectors.toList());

            clusterPhyContext.setAssociatedDataNodeNum(dataNodes.size());
            clusterPhyContext.setAssociatedDataNodeIps(dataNodes.stream().map(RoleClusterHostInfo::getIp).collect(Collectors.toList()));
            clusterPhyContext.setAssociatedNodeIps(hostList.stream().map(RoleClusterHostInfo::getIp).collect(Collectors.toList()));
            clusterPhyContext.setAssociatedRacks(
                dataNodes.stream().map(RoleClusterHostInfo::getRack).distinct().collect(Collectors.toList()));
    ...
    private void flushClusterLogicContexts() {
        ...
        Map<String, List<RoleClusterHostInfo>> phyRack2HostListMap = getPhyRack2HostListMap();
        ...
        for (ClusterLogic clusterLogic : clusterLogics) {
            ...
            List<RoleClusterHostInfo> associatedRackClusterHosts = Lists.newArrayList();
            for (ClusterRegion clusterRegion : clusterRegions) {
                ...
                for (String rack : rackList) {
                    List<RoleClusterHostInfo> associatedHosts = phyRack2HostListMap.get(clusterRegion.getPhyClusterName() + "@" + rack);
            ...
            //设置数据节点Ip地址
            clusterLogicContext.setAssociatedDataNodeIps(associatedRackClusterHosts.stream().map(RoleClusterHostInfo::getIp)
                    .collect(Collectors.toList()));
     ...
     */
    @NotNull
    private Map<String, List<RoleClusterHostInfo>> getPhyRack2HostListMap() {
        List<RoleClusterHostInfo> roleClusterHostInfos = roleClusterHostInfoService.listAllNode();
        return ConvertUtil.list2MapOfList(roleClusterHostInfos,
    ...
    private void setRegionAndAssociatedClusterPhyDataNodeInfo(ClusterLogicContext build) {
        ...
        for (ClusterRegion region : regions) {
            List<RoleClusterHostInfo> roleClusterHostInfos = roleClusterHostInfoService.listRacksNodes(region.getPhyClusterName(), region.getRacks());
            associatedRackClusterHosts.addAll(roleClusterHostInfos);
        }
        ...
        //设置数据节点Ip地址
        build.setAssociatedDataNodeIps(associatedRackClusterHosts.stream().map(RoleClusterHostInfo::getIp).collect(Collectors.toList()));
    }
    ...
    private void setClusterPhyNodeInfo(ClusterPhyContext build) {
        List<RoleClusterHostInfo> nodes = roleClusterHostInfoService.getNodesByCluster(build.getClusterName());
        List<RoleClusterHostInfo> dataNodes = nodes.stream().filter(r -> DATA_NODE.getCode() == r.getRole())
            .collect(Collectors.toList());

        build.setAssociatedDataNodeNum(dataNodes.size());
        build.setAssociatedDataNodeIps(dataNodes.stream().map(RoleClusterHostInfo::getIp).collect(Collectors.toList()));
        build.setAssociatedNodeIps(nodes.stream().map(RoleClusterHostInfo::getIp).collect(Collectors.toList()));
