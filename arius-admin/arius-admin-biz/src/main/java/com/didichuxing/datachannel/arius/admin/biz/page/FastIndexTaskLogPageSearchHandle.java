...
public class FastIndexTaskLogPageSearchHandle extends
    ...
    private static final String FAST_DUMP_SUCC_CODE = "200";
    ...
    private List<FastDumpTaskLogVO> listMysqlLogMessage(FastIndexLogsConditionDTO condition) {
        ...
        List<FastDumpTaskLogVO> fastDumpTaskLogList = fastDumpErrorLogs.stream().map(fastIndexTaskInfo -> {
            ...
            if (fastDumpCheckErrorLogs.size() != 0) {
                ...
                String reason = FAST_DUMP_SUCC_CODE.equals(jsonObject.getString("code")) ? "内核未知错误"
                        : jsonObject.getString("message");
                String message = String.format("【%s】,源集群【%s】，源索引【%s】的数据迁移任务提交失败，失败原因【%s】", AriusDateUtils.date2Str(fastIndexTaskInfo.getTaskEndTime(),FORMAT),
                        fastIndexDTO.getSourceCluster(), fastIndexTaskInfo.getIndexName(), reason);
                fastDumpTaskLogVO.setMessage(message);
            } else {
                JSONObject jsonObject = JSONObject.parseObject(fastIndexTaskInfo.getLastResponse());
                String reason = FAST_DUMP_SUCC_CODE.equals(jsonObject.getString("code")) ? "内核未知错误"
                        : jsonObject.getString("message");
                String message = String.format("【%s】,源集群【%s】，源索引【%s】的数据迁移任务状态刷新失败，失败原因【%s】", AriusDateUtils.date2Str(fastIndexTaskInfo.getTaskEndTime(),FORMAT),
                        fastIndexDTO.getSourceCluster(), fastIndexTaskInfo.getIndexName(), reason);
                fastDumpTaskLogVO.setMessage(message);
            }
