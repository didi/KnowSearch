...
public class OpTaskManagerImpl implements OpTaskManager {
    ...
    public Result<List<WorkTaskVO>> addTaskESExpand(ESClusterExpandWithPluginDTO data, String operator,
            ...
            Optional.ofNullable(data.getInstallPlugins()).orElse(Collections.emptyList()).stream()
                .map(JSON::toJSONString).map(
                    content -> {
                        ...
                        opTaskDTO.setTaskType(OpTaskTypeEnum.ES_PLUGIN_CLUSTER_EXPAND.getType());
        ...
        List<WorkTaskVO> workTaskVOS= Lists.newArrayList();
        Result<OpTask> result = addTask(opTaskDTO, AuthConstant.SUPER_PROJECT_ID);
        if (result.failed()) {
            return Result.buildFail(result.getMessage());
        }
        workTaskVOS.add(ConvertUtil.obj2Obj(result.getData(), WorkTaskVO.class));
        // 生成安装插件的任务
        for (OpTaskDTO taskDTO : pluginInstallList) {
            final Result<OpTask> opTaskResult = addTask(taskDTO, AuthConstant.SUPER_PROJECT_ID);
            if (opTaskResult.failed()) {
                return Result.buildFail(opTaskResult.getMessage());
            }
            workTaskVOS.add(ConvertUtil.obj2Obj(opTaskResult.getData(), WorkTaskVO.class));
        }
    ...
    public Result<List<WorkTaskVO>> addTaskESShrink(ESClusterShrinkWithPluginDTO data, String operator,
        ...
        final List<OpTaskDTO> pluginInstallList =
                ...
                content -> {
                    final OpTaskDTO opTaskDTO = new OpTaskDTO();
                    opTaskDTO.setTaskType(OpTaskTypeEnum.ES_PLUGIN_CLUSTER_SHRINK.getType());
                    opTaskDTO.setCreator(operator);
                    opTaskDTO.setExpandData(content);
                    return opTaskDTO;
                }).collect(Collectors.toList());
