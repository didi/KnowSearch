...
public class ESClusterPluginUpgradeTaskHandler extends AbstractESTaskHandler {
		...
		protected String getTitle(String expandData) {
				...
				final String  version   = packageService.getPackageById(packageId.longValue()).getData()
						.getVersion();
				final Integer packageIdBefore = componentService.queryComponentById(
						content.getComponentId()).getData().getPackageId();
				final String versionBefore = packageService.getPackageById(packageIdBefore.longValue())
						.getData().getVersion();
				// 获取组建名称
				final String name = componentService.queryComponentNameById(content.getComponentId()).getData();
				return String.format("%s- 集群名称【%s】- 插件名称【%s】版本升级【%s-%s】", operationType().getMessage(),
						dependComponentName
						, name, versionBefore, version);
		}
