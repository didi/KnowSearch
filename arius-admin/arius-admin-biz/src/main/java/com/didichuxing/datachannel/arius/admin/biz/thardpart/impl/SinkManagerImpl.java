...
public class SinkManagerImpl implements SinkManager {
    ...
    public Result<SinkSdkTemplateDeployInfoVO> listDeployInfo(@RequestParam(value = "templateName") String templateName) {
        // 主键顺序存放
        Result result = getIndexTemplateMetaByName(templateName);
        if (result.failed()) {
            return result;
        }

        IndexTemplateLogicWithPhyTemplates templateLogicWithPhysical = (IndexTemplateLogicWithPhyTemplates) result.getData();
     ...
     * @return
     */
    private Result getIndexTemplateMetaByName(String templateName) {
        // 主键顺序存放
     ...
     * @return
     */
    private Set<String> getPipelineClusters() {
        return ariusConfigInfoService.stringSettingSplit2Set(ARIUS_COMMON_GROUP,
    ...
    private SinkSdkAppVO buildAppAuthInfo(App app, List<AppTemplateAuth> appTemplateAuths,
        ...
        if (app.getIsRoot().equals( AdminConstant.YES)) {
            ...
            rIndexExpressSet.addAll(appTemplateAuths.stream()
                    .map(
                            auth -> templateId2IndexTemplateLogicMap.get(Integer.valueOf(auth.getTemplateId())).getExpression())
