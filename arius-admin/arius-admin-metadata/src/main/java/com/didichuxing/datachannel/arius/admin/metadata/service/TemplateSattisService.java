...
public class TemplateSattisService {
    ...
    public Result<IndexTemplateInfoWithStats> getTemplateBaseStatsInfo(Long logicTemplateId, Long startDate, Long endDate) {
        if (logicTemplateId == null) {
            return Result.build(ResultType.ILLEGAL_PARAMS);
        }

        IndexTemplateInfo indexTemplate = indexTemplateInfoService.getLogicTemplateById(logicTemplateId.intValue());
        if (null == indexTemplate) {
            return Result.buildFail();
        }

        IndexTemplateInfoWithStats indexTemplateLogicWithStats = (IndexTemplateInfoWithStats)indexTemplate;
        FutureUtil.DEAULT_FUTURE.runnableTask(() -> {
