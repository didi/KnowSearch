...
public class ESPhyPhyClusterRegionTest extends BasePhyClusterRegionInfoTest {
    ...
    public void getRegionNodesTest() throws IOException {
        Result<List<ESRoleClusterHostVO>> result = ESPhyClusterRegionControllerMethod.getRegionNodes(phyClusterRegionInfo.getRegionId());
        Assertions.assertTrue(result.success());
        if(result.success()) {
            Set<String> rack = result.getData().stream().map(ESRoleClusterHostVO::getRack).collect(Collectors.toSet());
