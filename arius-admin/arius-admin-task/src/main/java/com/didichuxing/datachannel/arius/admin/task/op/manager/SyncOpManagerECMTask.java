...
public class SyncOpManagerECMTask implements Job {
    private static final ILog LOGGER = LogFactory.getLog(SyncOpManagerECMTask.class);
		@Autowired
		...
		public TaskResult execute(JobContext jobContext) throws Exception {
				...
				for (OpTask opTask : opTasks) {
						...
						try {
								...
								if (result.failed()) {
										LOGGER.warn("任务ID【{}】状态更新失败,原因：【{}】", opTask.getId(),
												result.getMessage());
										errorMessage.add(
												String.format("任务ID【%s】状态更新失败,原因：【%s】", opTask.getId(),
														result.getMessage()));
								}
						} catch (Exception e) {
								LOGGER.error("任务ID【{}】状态更新失败", opTask.getId(), e);
						}
				}
				TaskResult taskResult = CollectionUtils.isEmpty(errorMessage) ? TaskResult.buildSuccess() :
				                        new TaskResult(TaskResult.FAIL_CODE,
