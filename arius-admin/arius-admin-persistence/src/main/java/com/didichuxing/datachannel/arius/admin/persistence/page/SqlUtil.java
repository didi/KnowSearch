...
public class SqlUtil {
    ...
    private static final ILog                            LOGGER             = LogFactory.getLog(SqlUtil.class);

    private static final ThreadLocal<Page>               LOCAL_PAGE         = new ThreadLocal<Page>();
    //总数缓存
    private static final ConcurrentHashMap<String, Long> TOTAL_COUNT        = new ConcurrentHashMap<>();
    //最大缓存数量
    ...
    //最大pagesize
    private static int                                   maxPageSize        = 10000;
    ...
    private static MetaObject forObject(Object object) {
        return MetaObject.forObject(object, DEFAULT_OBJECT_FACTORY, DEFAULT_OBJECT_WRAPPER_FACTORY,
                DEFAULT_OBJECT_REFLECTOR_FACTORY);
    }
    ...
    //具体针对数据库的parser
    private SqlUtil.Parser parser;
    //数据库方言
    private SqlUtil.Dialect dialect;
    ...
    private Object _processPage(Invocation invocation) throws Throwable {
        ...
        if (SqlUtil.getLocalPage() == null && rowBounds == RowBounds.DEFAULT) {
            ...
            if (page.isCount() && page.getPageNum() == 1) {
                ...
                String key = getKeyFromObject(args[1], sqlSource);
                if (StringUtils.isNotBlank(key)) {
                    if (TOTAL_COUNT.size() >= MAX_CACHE_SIZE) {
                        TOTAL_COUNT.clear();
                    }
                    TOTAL_COUNT.put(key, page.getTotal());
                }
                if (page.getTotal() == 0) {
                    return page;
                }
                ...
                String key = getKeyFromObject(args[1], sqlSource);
                if (StringUtils.isNotBlank(key)) {
                    total = TOTAL_COUNT.get(getKeyFromObject(args[1], sqlSource));
                }
                if (total != null) {
                    page.setTotal(total);
                } else {
                    //如果之前没有查过，或者key生成失败，则再查一次
                    Object result = invocation.proceed();
                    //设置总数
                    page.setTotal((Integer) ((List) result).get(0));
                    if (StringUtils.isNotBlank(key)) {
    ...
    private String getKeyFromObject(Object args1, SqlSource sqlSource) {
        try {
            Map<String, Object> argMap = JSON.parseObject(JSON.toJSONString(args1), Map.class);
            argMap.remove("pn");
            argMap.remove("ps");
            argMap.remove("skip");
            argMap.remove("max");
            String key = sqlSource.getBoundSql(args1).getSql() + ":" + JSON.toJSONString(argMap);
            return key;
        } catch (Exception ex) {
            LOGGER.error("generator key error, args1=" + args1 + ";sql=" + sqlSource.getBoundSql(args1).getSql());
            return "";
        }
    }
    ...
    public void setProperties(Properties p) {
        ...
        String defaultPageSize = p.getProperty("defaultPageSize");
        String maxPageSize = p.getProperty("maxPageSize");
        if (defaultPageSize != null) {
            SqlUtil.defaultPageSize = Integer.valueOf(defaultPageSize);
        }
        if (maxPageSize != null) {
            SqlUtil.maxPageSize = Integer.valueOf(maxPageSize);
        }
        //参数映射
    ...
    public SqlUtil(String strDialect) {
        ...
        try {
            dialect = SqlUtil.Dialect.valueOf(strDialect);
            parser = SqlUtil.SimpleParser.newParser(dialect);
        } catch (IllegalArgumentException e) {
            String dialects = null;
            for (SqlUtil.Dialect d : SqlUtil.Dialect.values()) {
                if (dialects == null) {
                    dialects = d.toString();
                } else {
                    dialects += "," + d;
        ...
        public static SqlUtil.Parser newParser(SqlUtil.Dialect dialect) {
            ...
            switch (dialect) {
                ...
                case hsqldb:
                    ...
                    break;
                case sqlserver:
                    parser = new SqlUtil.SqlServerParser();
                    break;
                case db2:
                    parser = new SqlUtil.Db2Parser();
                    break;
                case postgresql:
                default:
    ...
    //SqlServer
    private static class SqlServerParser extends SqlUtil.SimpleParser {
    ...
    //Db2Parser
    private static class Db2Parser extends SqlUtil.SimpleParser {
        @Override
        public String getPageSql(String sql) {
            StringBuilder sqlBuilder = new StringBuilder(sql.length() + 120);
    ...
    private SqlSource getsqlSource(MappedStatement ms, SqlSource sqlSource, Object parameterObject, String suffix) {
        ...
        if (isDynamic(ms)) {
            ...
            return new SqlUtil.MyDynamicSqlSource(ms.getConfiguration(), mixedSqlNode, suffix == SUFFIX_COUNT);
        } else if (sqlSource instanceof ProviderSqlSource) {
            return new SqlUtil.MyProviderSqlSource(ms.getConfiguration(), (ProviderSqlSource) sqlSource,
                    suffix == SUFFIX_COUNT);
        }
        //如果是静态分页sql
    ...
    public static void testSql(String dialet, String originalSql) {
        ...
        if (sqlUtil.dialect == SqlUtil.Dialect.sqlserver) {
