...
public class GatewayServiceTest {
    ...
    void sqlOperateTest() {
        ...
        Assertions.assertEquals("参数错误:查询的sql语句为空，请检查后再提交！",
                gatewayService.sqlOperate("", CustomDataSource.PHY_CLUSTER_NAME, esUser, "postFix").getMessage());
        Assertions.assertEquals("参数错误:查询gateway的路径后缀为空，请检查后再提交！",
                gatewayService.sqlOperate(sql, CustomDataSource.PHY_CLUSTER_NAME, esUser, null).getMessage());
        
        Mockito.when(esGatewayClient.getSingleGatewayAddress()).thenReturn("10.190.32.30");
        Assertions.assertFalse(
                gatewayService.sqlOperate("sql", CustomDataSource.PHY_CLUSTER_NAME, esUser, "postFix").success());
    ...
    void getAliveNodeTest() {
        when(gatewayClusterNodeDAO.listAliveNodeByClusterNameAndTime(any(), any())).thenReturn(
                CustomDataSource.getGatewayNodePOList());
