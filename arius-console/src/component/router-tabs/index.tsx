...
export const RouterTabs = withRouter<any, any>((props: { tabList: ITab[], removePaths: string[], permissions }) => {
  ...
  React.useEffect(() => {
    ...
    const defaultList = [{ key: 'menu.es.cluster.physics', label: '物理集群', href: '/cluster/physics', show: true }];
    const newTabList = (window as any).currentOpenRouterList.filter(item => !props.permissions[item.key]);
    const currentOpenRouterList = newTabList.length ? newTabList : defaultList;

    (window as any).currentOpenRouterList = currentOpenRouterList;

    // 判断当前页面被选中页面，是否是详情页
    const isDetailIndx = currentOpenRouterList.findIndex(item => item.key === currentKey);
    
    if(isDetailIndx === -1) {
      setCurrentKey(currentOpenRouterList[0].key);
      (props as any).history.push(currentOpenRouterList[0].href);
    }
  }, [department]);
