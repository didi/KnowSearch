...
const TimeModule: React.FC<propsType> = ({ timeChange, rangeTimeArr, popoverClassName }) => {
  ...
  const clickContent = <div className="dd-time-range-module">
    {/* <span>时间：</span> */}
        ...
        >
          <Space direction="vertical" size={16}>
            {TimeOptions.map((item, index) => (
              <Radio.Button value={item.value} key={index}>{item.label}</Radio.Button>
            ))}
          </Space>
        </Radio.Group>
      </div>
    ...
    <>
      <div id="d-range-time">
        <Popover trigger={['click']}
          ...
          content={clickContent} placement="bottomRight" overlayClassName={`d-range-time-popover ${popoverClassName ?? ''}`}>
