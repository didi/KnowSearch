...
export const getOperationColumns = (setDrawerId) => {
  let cols = [
    ...
    {
      title: "资源所属应用",
      dataIndex: "operateProjectName",
      key: "operateProjectName",
      width: "10%",
      render: (val: string) => val || "-",
    },
    {
    ...
    },
    {
      ...
      render: (val: string, record) => {
        let diffArr = ["编辑MAPPING", "编辑SETTING", "配置文件变更", "配置变更", "集群平台插件配置变更", "Gateway配置文件变更"];
        if (diffArr.includes(record.operate)) {
          ...
          if (record.operate === "编辑MAPPING") {
            ...
            operate = "配置文件";
          } else if (record.operate === "配置变更") {
            operate = "配置";
          } else if (record.operate === "集群平台插件配置变更") {
            operate = "集群平台插件配置";
          } else if (record.operate === "Gateway配置文件变更") {
            operate = "Gateway配置文件";
          }
