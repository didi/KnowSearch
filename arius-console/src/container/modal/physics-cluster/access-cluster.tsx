  ...
  const xFormModalConfig = () => {
    let formMap = [
      ...
      },
      {
        ...
        customFormItem: <Senior type="access" />,
      },
    ] as IFormItem[];
    if (isFullType) {
      let fullnode = [
    ...
    return {
      ...
      onSubmit: async (result) => {
        ...
        let password = result.usename && result.password ? `${result.usename}:${result.password}` : "";
        const params = {
          type: 4,
          resourceType: Number(result?.clusterType) || 2, // 集群类型默认为独立集群
