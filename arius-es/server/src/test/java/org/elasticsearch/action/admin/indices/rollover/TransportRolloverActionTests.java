...
public class TransportRolloverActionTests extends ESTestCase {
    ...
    public void testGenerateRolloverIndexName() {
        ...
        assertThat(invalidIndexName + "_v1",
            equalTo(TransportRolloverAction.generateRolloverIndexName(invalidIndexName, indexNameExpressionResolver)));
        int num = randomIntBetween(0, 100);
        final String indexPrefix = randomAlphaOfLength(10);
        String indexEndingInNumbers = indexPrefix + "_v" + num;
        assertThat(TransportRolloverAction.generateRolloverIndexName(indexEndingInNumbers, indexNameExpressionResolver),
            equalTo(indexPrefix + "_v" + String.format(Locale.ROOT, "%d", num + 1)));
        assertThat(TransportRolloverAction.generateRolloverIndexName("index-name_v1", indexNameExpressionResolver),
            equalTo("index-name_v2"));
        assertThat(TransportRolloverAction.generateRolloverIndexName("index-name_v2", indexNameExpressionResolver),
            equalTo("index-name_v3"));
        assertEquals( "<index-name-{now/d}_v2>", TransportRolloverAction.generateRolloverIndexName("<index-name-{now/d}_v1>",
    ...
    public void testConditionEvaluationWhenAliasToWriteAndReadIndicesConsidersOnlyPrimariesFromWriteIndex() throws Exception {
        ...
        final TransportRolloverAction transportRolloverAction = new TransportRolloverAction(mockTransportService, mockClusterService,
                ...
                mock(IngestService.class), mockClient);
