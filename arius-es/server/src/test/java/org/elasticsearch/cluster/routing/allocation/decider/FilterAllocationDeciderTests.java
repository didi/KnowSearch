...
public class FilterAllocationDeciderTests extends ESAllocationTestCase {
    ...
    public void testFilterInitialRecovery() {
        ...
        ShardRouting shardRouting = routingTable.index("idx").shard(0).shards().get(1);
        assertEquals(shardRouting.state(), INITIALIZING);
        assertEquals(shardRouting.currentNodeId(), "node2");
        routingTable = service.applyFailedShard(state, shardRouting, randomBoolean()).routingTable();
