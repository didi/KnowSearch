...
public class PrioritizedEsThreadPoolExecutor extends EsThreadPoolExecutor {
    ...
    protected Runnable wrapRunnable(Runnable command) {
        if (command instanceof PrioritizedRunnable) {
            ...
            long insertionOrder = this.insertionOrder.incrementAndGet();
            ((PrioritizedRunnable) command).setInsertionOrder(insertionOrder);
            return new TieBreakingPrioritizedRunnable(super.wrapRunnable(command), priority, insertionOrder);
    ...
    public void remove(Collection<TaskBatcher.BatchedTask> batchedTasks) {
        final List<Object> toRemove = new ArrayList<>();
        for (Runnable runnable : getQueue()) {
            if (runnable instanceof TieBreakingPrioritizedRunnable) {
                TieBreakingPrioritizedRunnable t = (TieBreakingPrioritizedRunnable) runnable;
                Runnable innerRunnable = t.runnable;
                if (innerRunnable != null && batchedTasks.contains(super.unwrap(innerRunnable))) {
