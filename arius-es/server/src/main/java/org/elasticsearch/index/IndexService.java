...
public class IndexService extends AbstractIndexComponent implements IndicesClusterStateService.AllocatedIndex<IndexShard> {
    ...
    public synchronized IndexShard createShard(
        ...
        try {
            ...
            if (path == null) {
                ...
                String shardGroup = indexSettings.get(INDEX_GROUP_NAME);
                Map<Path, Float> dataPathToFactor = new HashMap<>();
                Map<Path, Integer> dataPathToShardCount = new HashMap<>();
                for (IndexShard shard : indicesService.getIndexShards().values()) {
                    Path dataPath = shard.shardPath().getRootStatePath();
                    String group = shard.indexSettings().getIndexMetaData().getSettings().get(INDEX_GROUP_NAME);
                    float factor = shard.indexSettings().getIndexMetaData().getSettings().getAsFloat(INDEX_GROUP_FACTOR, 0.1f);
                    if (group != null && group.equals(shardGroup)) {
                        Float totalFactor = dataPathToFactor.get(dataPath);
                        if (totalFactor == null) {
                            totalFactor = 0f;
                        }
                        dataPathToFactor.put(dataPath, totalFactor + factor);
                    }
                ...
                path = ShardPath.selectNewPathForShard(nodeEnv, shardId, this.indexSettings,
                    ...
                    dataPathToShardCount, dataPathToFactor);
            ...
            indicesService.getIndexShards().put(shardId, indexShard);
            return indexShard;
    ...
    public synchronized void removeShard(int shardId, String reason) {
        ...
        indicesService.getIndexShards().remove(sId);
    }
