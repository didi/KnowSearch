...
final class TranslogSnapshot extends BaseTranslogReader {
    private static final Logger logger = LogManager.getLogger(TranslogSnapshot.class);
    ...
    public Translog.Operation next() throws IOException {
        ...
        if (checkpoint.offset < length) {
            try {
                ...
                logger.warn(String.format(Locale.US, "read eof log, position=%d, length=%d", position, length), e);
                return null;
            } catch (TranslogCorruptedException e) {
                logger.warn(String.format(Locale.US,"read corrented log, position=%d, length=%d", position, length), e);
                return null;
            } catch (Exception e) {
                logger.warn(String.format(Locale.US,"unexcepted exception, position=%d, length=%d", position, length), e);
                return null;
            }
        }
