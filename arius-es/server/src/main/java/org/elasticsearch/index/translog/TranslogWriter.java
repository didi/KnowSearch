...
public class TranslogWriter extends BaseTranslogReader implements Closeable {
    ...
    private volatile boolean translogFSync = true;
        ...
        if (lastSyncedCheckpoint.offset < offset && syncNeeded()) {
            synchronized (syncLock) { // only one sync/checkpoint should happen concurrently but we wait
                if (lastSyncedCheckpoint.offset < offset && syncNeeded()) {
                    ...
                    try {
                        if (this.translogFSync) {
                            channel.force(false);
                        }
                        writeCheckpointBySyncMode(channelFactory, path.getParent(), checkpointToSync, this.translogFSync);
                    } catch (final Exception ex) {
                        closeWithTragicEvent(ex);
                        throw ex;
                    }
    ...
    private static void writeCheckpointBySyncMode(
        final ChannelFactory channelFactory,
        final Path translogFile,
        final Checkpoint checkpoint, boolean translogFSync) throws IOException {
        Checkpoint.writeBySyncMode(channelFactory, translogFile.resolve(Translog.CHECKPOINT_FILE_NAME), checkpoint, translogFSync, StandardOpenOption.WRITE);
    }
