...
public final class ShardPath {
    ...
    private final Path rootStatePath;
    ...
    public ShardPath(boolean isCustomDataPath, Path dataPath, Path shardStatePath, ShardId shardId) {
        ...
        this.rootStatePath = shardStatePath.getParent().getParent().getParent();
    }
    ...
    public Path getRootStatePath() {
        return rootStatePath; // also strip the indices folder
    }
    ...
    public static ShardPath selectNewPathForShard(NodeEnvironment env, ShardId shardId, IndexSettings indexSettings,
                                                  long avgShardSizeInBytes, Map<Path,Integer> dataPathToShardCount, Map<Path, Float> dataPathToFactor) throws IOException {
            ...
            if (paths.length != 1) {
                ...
                bestPath = Arrays.stream(paths)
                        ...
                        .sorted((p1, p2) -> {
                                ...
                                cmp = Float.compare(dataPathToFactor.getOrDefault(p1.path, 0f),
                                    dataPathToFactor.getOrDefault(p2.path, 0f));
                                if (cmp != 0) {
                                    return cmp;
                                }
                                ...
                                return pathsToSpace.get(p2).compareTo(pathsToSpace.get(p1));
                            })
