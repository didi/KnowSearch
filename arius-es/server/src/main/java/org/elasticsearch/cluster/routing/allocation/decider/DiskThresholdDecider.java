...
public class DiskThresholdDecider extends AllocationDecider {
    ...
    private final DiskThresholdSettings coldDiskThresholdSettings;
    ...
    public Decision canAllocate(ShardRouting shardRouting, RoutingNode node, RoutingAllocation allocation) {
        DiskThresholdSettings diskThresholdSettings = getDiskThresholdSettings(node);
        ...
        if (freeBytes < diskThresholdSettings.getFreeBytesThresholdLow().getBytes()) {
            if (skipLowThresholdChecks == false) {
                ...
                return allocation.decision(Decision.NO, NAME,
                    ...
                    "space, actual free: [%s]",
                    diskThresholdSettings.getLowWatermarkSetting().getKey(),
                ...
                return allocation.decision(Decision.NO, NAME,
                    ...
                    "space, actual free: [%s]",
                    diskThresholdSettings.getHighWatermarkSetting().getKey(),
    ...
    private Decision earlyTerminate(RoutingAllocation allocation, ImmutableOpenMap<String, DiskUsage> usages) {
        ...
        if (hotDiskThresholdSettings.isEnabled() == false) {
    ...
    private DiskThresholdSettings getDiskThresholdSettings(RoutingNode node) {

        if(isColdNode(node)) {
            return coldDiskThresholdSettings;
        } else {
            return hotDiskThresholdSettings;
        }
    };

    private boolean isColdNode(RoutingNode node) {
        if(node==null) {
            return false;
        }
        ...
        if(discoveryNode!=null) {
            return discoveryNode.isColdNode();
        }
