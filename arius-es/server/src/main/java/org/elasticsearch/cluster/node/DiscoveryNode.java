...
public class DiscoveryNode implements Writeable, ToXContentFragment {
    ...
    public boolean isColdNode() {
        return isColdNode(attributes);
    }

    public static boolean isColdNode(Settings nodeSettings) {
        Map<String, String> attrMap = Node.NODE_ATTRIBUTES.getAsMap(nodeSettings);
        return isColdNode(attrMap);
    }

    private static boolean isColdNode(Map<String, String> attributes) {
        if(attributes==null) {
            return false;
        }

        String set = attributes.get("set");
        if (set== null) {
            return false;
        }

        if (set.equalsIgnoreCase("cold")) {
            return true;
        }
