    ...
    class PutMappingExecutor implements ClusterStateTaskExecutor<PutMappingClusterStateUpdateRequest> {
        ...
        private ClusterState applyRequest(ClusterState currentState, PutMappingClusterStateUpdateRequest request,
            ...
            for (Index index : request.indices()) {
                ...
                // TODO 临时的逻辑，可以删除多type索引下的某个type
                if (indexMetaData.getMappings().size() > 1
                    && indexMetaData.getMappings().containsKey(request.type())
                    && request.source().equals("{}")
                    && request.indices().length == 1) {
                    MetaData.Builder builder = MetaData.builder(metaData);
                    IndexMetaData.Builder indexMetaDataBuilder = IndexMetaData.builder(indexMetaData);
                    indexMetaDataBuilder.removeMapping(request.type());
                    builder.put(indexMetaDataBuilder);
                    logger.warn("remove-mapping, remove index({}) type({})", index, request.type());
