...
public class MasterService extends AbstractLifecycleComponent {
    ...
    public static final Setting<List<String>> MASTER_SERVICE_CANCEL_PENDING_TASKS_SETTING =
        Setting.listSetting("cluster.service.cancel.pending_tasks", emptyList(), Function.identity(),
            Setting.Property.Dynamic, Setting.Property.NodeScope);
    ...
    public MasterService(Settings settings, ClusterSettings clusterSettings, ThreadPool threadPool) {
        ...
        clusterSettings.addSettingsUpdateConsumer(MASTER_SERVICE_CANCEL_PENDING_TASKS_SETTING, this::cancelPendingTasks);
    ...
    private void cancelPendingTasks(List<String> sources) {
        if (sources == null || sources.isEmpty()) {
            return;
        }
        List<TaskBatcher.BatchedTask> canRemove = Arrays.stream(threadPoolExecutor.getPending()).filter(pending -> {
            ...
            for (String source : sources) {
                if (Strings.hasText(source) && task.source().contains(source)) {
                    return true;
                }
            }
            return false;
        }).map(pending -> (TaskBatcher.BatchedTask) pending.task).collect(Collectors.toList());
        taskBatcher.cancelPendingTasks(canRemove);
    }
    ...
    class Batcher extends TaskBatcher {
        ...
        class UpdateTask extends BatchedTask {
            ...
            UpdateTask(Priority priority, String source, Object task, ClusterStateTaskListener listener,
                ...
                super(priority, source, executor, task, new TaskState(clusterStateSupplier.get(), source, priority));
    ...
    private void runTasks(TaskInputs taskInputs) {
        ...
        if (!previousClusterState.nodes().isLocalNodeElectedMaster() && taskInputs.runOnlyWhenMaster()) {
            ...
            logTaskStates(taskInputs, "failure", "local node is no longer master", false);
            return;
        }

        final long computationStartTime = threadPool.relativeTimeInMillis();
        long executeStartTimeMillis = System.currentTimeMillis();
        final TaskOutputs taskOutputs = calculateTaskOutputs(taskInputs, previousClusterState);
        long executeEndTimeMillis = System.currentTimeMillis();
        taskInputs.updateTasks.forEach(t -> t.taskState.setExecute(executeEndTimeMillis - executeStartTimeMillis));
        ...
        if (taskOutputs.clusterStateUnchanged()) {
            ...
            logTaskStates(taskInputs, "success", null, false);
        } else {
            ...
            try {
                ...
                logTaskStates(taskInputs, "success", null, true);
            } catch (Exception e) {
                handleException(summary, publicationStartTime, newClusterState, e);

                logTaskStates(taskInputs, "failure", e.getMessage(), true);
            }
        }
    }

    private void logTaskStates(TaskInputs taskInputs, String taskResult, String failure, boolean clusterStateChanged) {
        long endTimeMillis = System.currentTimeMillis();
        taskInputs.updateTasks.forEach(t -> {
            t.taskState.setTaskResult(taskResult);
            if (Strings.hasText(failure)) {
                t.taskState.setFailure(failure);
            }
            t.taskState.setClusterStateChanged(clusterStateChanged);
            t.taskState.setProcess(endTimeMillis - t.taskState.getProcessTimeMillis());
            t.taskState.setTotal(endTimeMillis - t.taskState.getSubmitTimeMillis());
            t.taskState.setTaskType(Strings.hasText(taskInputs.executor.taskType()) ? taskInputs.executor.taskType() : t.source);
            String message = t.taskState.toString();
    ...
    protected void publish(ClusterChangedEvent clusterChangedEvent, TaskOutputs taskOutputs, long startTimeMillis) {
        final PlainActionFuture<TaskState> fut = new PlainActionFuture<TaskState>() {
            @Override
            protected boolean blockingAllowed() {
                return isMasterUpdateThread() || super.blockingAllowed();
            }
        };
        long publishStartTimeMillis = System.currentTimeMillis();
        ...
        try {
            TaskState taskState = FutureUtils.get(fut);
            long publishEndTimeMillis = System.currentTimeMillis();
            taskOutputs.taskInputs.updateTasks.forEach(t -> {
                t.taskState.setPublish(publishEndTimeMillis - publishStartTimeMillis);
                if (taskState != null) {
                    t.taskState.setAppliers(taskState.getAppliers());
                    t.taskState.setListeners(taskState.getListeners());
                }
            });
