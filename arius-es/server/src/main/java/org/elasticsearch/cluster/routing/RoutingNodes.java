...
public class RoutingNodes implements Iterable<RoutingNode> {
    ...
    private final Map<String, List<RoutingNode>> nodesPerRegionNames = new HashMap<>();
    ...
    public RoutingNodes(ClusterState clusterState, boolean readOnly) {
        ...
        this.unassignedShards.setConcurrentRecoveries(clusterState);
            ...
            for (IndexShardRoutingTable indexShard : indexRoutingTable.value) {
                ...
                for (ShardRouting shard : indexShard) {
                    ...
                    if (shard.assignedToNode()) {
                        ...
                        unassignedShards.add(shard, true);
                    }
                }
            }
        }

        unassignedShards.mergeGatewayUnassignedShards();
        ...
        for (Map.Entry<String, LinkedHashMap<ShardId, ShardRouting>> entry : nodesToShards.entrySet()) {
            ...
            RoutingNode routingNode = new RoutingNode(clusterState, nodeId, discoveryNode, entry.getValue());
            this.nodesToShards.put(nodeId, routingNode);

            if (discoveryNode != null) {
                String regionName = discoveryNode.getAttributes().getOrDefault(INDEX_SETTING_REGION_ATTRIBUTE, INDEX_SETTING_DEFAULT_REGION_NAME);
                nodesPerRegionNames.computeIfAbsent(regionName, k -> new ArrayList<>()).add(routingNode);
            }
        }
    }
    ...
    public Tuple<ShardRouting,ShardRouting> relocateShard(ShardRouting startedShard, String nodeId, long expectedShardSize,
        ...
        addRecovery(target);
        changes.relocationStarted(startedShard, source, target);
    ...
    public ShardRouting startShard(Logger logger, ShardRouting initializingShard, RoutingChangesObserver routingChangesObserver) {
        ...
        if (initializingShard.relocatingNodeId() != null) {
            ...
            remove(relocationSourceShard, routingChangesObserver);
                ...
                for (ShardRouting routing : new ArrayList<>(assignedShards)) {
                    if (routing.initializing() && routing.primary() == false) {
                        if (routing.isRelocationTarget()) {
                            ...
                            ShardRouting startedReplica = cancelRelocation(sourceShard, routingChangesObserver);
                            remove(routing, routingChangesObserver);
    ...
    public void failShard(Logger logger, ShardRouting failedShard, UnassignedInfo unassignedInfo, IndexMetaData indexMetaData,
        ...
        if (failedShard.relocating()) {
            ...
            if (failedShard.primary()) {
                ...
                remove(targetShard, routingChangesObserver);
                ...
                removeRelocationSource(targetShard, routingChangesObserver);
        ...
        if (failedShard.initializing()) {
            if (failedShard.relocatingNodeId() == null) {
                if (failedShard.primary()) {
                    ...
                    moveToUnassigned(failedShard, unassignedInfo, routingChangesObserver);
                }
            } else {
                ...
                logger.trace("{}, resolved source to [{}]. canceling relocation ... ({})", failedShard.shardId(), sourceShard,
                    unassignedInfo.shortSummary());
                cancelRelocation(sourceShard, routingChangesObserver);
                remove(failedShard, routingChangesObserver);
            }
        } else {
    ...
    private void unassignPrimaryAndPromoteActiveReplicaIfExists(ShardRouting failedShard, UnassignedInfo unassignedInfo,
        ...
        if (activeReplica == null) {
            ...
            movePrimaryToUnassignedAndDemoteToReplica(failedShard, unassignedInfo, routingChangesObserver);
    ...
    private void promoteReplicaToPrimary(ShardRouting activeReplica, RoutingChangesObserver routingChangesObserver) {
        ...
        ShardRouting primary = promoteActiveReplicaShardToPrimary(activeReplica);
        routingChangesObserver.replicaPromoted(activeReplica, primary);
    }
     ...
     */
    private ShardRouting cancelRelocation(ShardRouting shard, RoutingChangesObserver routingChangesObserver) {
        relocatingShards--;
        ShardRouting cancelledShard = shard.cancelRelocation();
        updateAssigned(shard, cancelledShard);
        routingChangesObserver.cancelRelocation(shard, cancelledShard);
     ...
     */
    private void remove(ShardRouting shard, RoutingChangesObserver routingChangesObserver) {
        ...
        if (shard.initializing() && shard.relocatingNodeId() == null) {
            ...
            shard = cancelRelocation(shard, routingChangesObserver);
        }
        routingChangesObserver.remove(shard);
     ...
     */
    private ShardRouting removeRelocationSource(ShardRouting shard, RoutingChangesObserver routingChangesObserver) {
        ...
        routingChangesObserver.removeRelocationSource(shard, relocationMarkerRemoved);
    ...
    private ShardRouting moveToUnassigned(ShardRouting shard, UnassignedInfo unassignedInfo, RoutingChangesObserver routingChangesObserver) {
        assert shard.unassigned() == false : "only assigned shards can be moved to unassigned (" + shard + ")";
        remove(shard, routingChangesObserver);
        ShardRouting unassigned = shard.moveToUnassigned(unassignedInfo);
        routingChangesObserver.unassignedInfoUpdated(shard, unassigned, unassignedInfo);
     ...
     */
    private ShardRouting movePrimaryToUnassignedAndDemoteToReplica(ShardRouting shard,
    ...
    public static final class UnassignedShards implements Iterable<ShardRouting>  {
        ...
        private final LinkedList<ShardRouting> gateway;
        private final LinkedList<ShardRouting> throttle;
        private final List<ShardRouting> unassigned;
        private final List<ShardRouting> ignored;

        private int primaries = 0;
        private int ignoredPrimaries = 0;
        private int concurrentRecoveries = 0;

        public UnassignedShards(RoutingNodes nodes) {
            this.nodes = nodes;
            gateway = new LinkedList<>();
            throttle = new LinkedList<>();
            unassigned = new ArrayList<>();
            ignored = new ArrayList<>();
        }

        public void setConcurrentRecoveries(ClusterState clusterState) {
            Settings settings = clusterState.metaData().settings();
            Integer primariesInitialRecoveries = CLUSTER_ROUTING_ALLOCATION_NODE_INITIAL_PRIMARIES_RECOVERIES_SETTING.get(settings);
            Integer concurrentIncomingRecoveries = CLUSTER_ROUTING_ALLOCATION_NODE_CONCURRENT_INCOMING_RECOVERIES_SETTING.get(settings);
            Integer concurrentOutgoingRecoveries = CLUSTER_ROUTING_ALLOCATION_NODE_CONCURRENT_OUTGOING_RECOVERIES_SETTING.get(settings);
            this.concurrentRecoveries = clusterState.nodes().getDataNodes().size() *
                Math.max(primariesInitialRecoveries, Math.max(concurrentIncomingRecoveries, concurrentOutgoingRecoveries));
        }

        public void mergeGatewayUnassignedShards() {
            // move throttle shards to gateway
            while (gateway.size() < concurrentRecoveries && throttle.isEmpty() == false) {
                gateway.add(throttle.removeFirst());
            }

            // ignore throttle left shards
            while (throttle.isEmpty() == false) {
                ShardRouting shardRouting = throttle.removeFirst();
                if (shardRouting.primary()) {
                    primaries--;
                    ignoredPrimaries++;
                }
                ignored.add(shardRouting);
            }

            unassigned.addAll(0, gateway);
            gateway.clear();
        }

        public static boolean isResponsibleFor(final ShardRouting shard) {
            return PrimaryShardAllocator.isResponsibleFor(shard) || ReplicaShardAllocator.isResponsibleFor(shard);
        }

        public void add(ShardRouting shardRouting) {
            add(shardRouting, false);
        }

        private void add(ShardRouting shardRouting, boolean initial) {
            if (shardRouting.primary()) {
                primaries++;
            }
            if (initial == true && concurrentRecoveries > 0 && isResponsibleFor(shardRouting)) {
                // primary first
                if (shardRouting.primary() && gateway.size() < concurrentRecoveries) {
                    gateway.add(shardRouting);
                } else if ((gateway.size() + throttle.size()) < concurrentRecoveries) {
                    throttle.add(shardRouting);
                } else {
                    // ignore
                    if (shardRouting.primary()) {
                        primaries--;
        ...
        public void ignoreShard(ShardRouting shard, AllocationStatus allocationStatus, RoutingChangesObserver changes) {
            ...
            if (shard.primary()) {
                ...
                if (allocationStatus.equals(currInfo.getLastAllocationStatus()) == false) {
                    ...
                    changes.unassignedInfoUpdated(shard, updatedShard, newInfo);
                    shard = updatedShard;
                }
            }
            ignored.add(shard);
        ...
        public class UnassignedIterator implements Iterator<ShardRouting> {
            ...
            public ShardRouting updateUnassigned(UnassignedInfo unassignedInfo, RecoverySource recoverySource,
                ...
                changes.unassignedInfoUpdated(current, updatedShardRouting, unassignedInfo);
    ...
    public Iterator<ShardRouting> nodeInterleavedShardIterator(List<RoutingNode> routingNodeList) {
        final Queue<Iterator<ShardRouting>> queue = new ArrayDeque<>();
        for (RoutingNode routingNode : routingNodeList) {
            queue.add(routingNode.copyShards().iterator());
        }
        return new Iterator<ShardRouting>() {
            public boolean hasNext() {
                while (!queue.isEmpty()) {
    ...
    public Tuple<String, List<RoutingNode>> randomRegion() {
        List<String> regionNames = new ArrayList<>(nodesPerRegionNames.keySet());
        Randomness.shuffle(regionNames);
        String regionName = regionNames.get(0);
        return Tuple.tuple(regionName, nodesPerRegionNames.get(regionName));
    }
}
