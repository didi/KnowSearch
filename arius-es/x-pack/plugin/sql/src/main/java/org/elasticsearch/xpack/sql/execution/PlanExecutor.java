...
public class PlanExecutor {
    ...
    public void searchSource(Configuration cfg, String sql, List<SqlTypedParamValue> params, ActionListener<SearchRequest> listener) {
        metrics.translate();

        newSession(cfg).sqlExecutable(sql, params, wrap(exec -> {
            if (exec instanceof EsQueryExec) {
                EsQueryExec e = (EsQueryExec) exec;
                SearchSourceBuilder searchSourceBuilder = SourceGenerator.sourceBuilder(e.queryContainer(), cfg.filter(), cfg.pageSize());
                SearchRequest searchRequest = new SearchRequest(e.index());
